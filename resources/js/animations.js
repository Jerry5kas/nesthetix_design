    ...
    initScrollAnimations() {
        ...
        Object.entries(animations).forEach(([name, props]) => {
            gsap.utils.toArray(`[data-animate="${name}"]`).forEach(el => {
                if (el.dataset.animateCustom === 'true') return;
        ...
        gsap.utils.toArray('[data-animate="stagger"]').forEach(container => {
            if (container.dataset.animateCustom === 'true') return;
            ...
            gsap.fromTo(container.children,
                { y: 40, opacity: 0 },
                {
                    ...
                    opacity: 1,
                    duration: 0.8,
                    stagger: parseFloat(container.dataset.stagger) || 0.15,
                    ease: 'power3.out'
                }
            );
        });
...
export default window.NesthetixAnimations;
